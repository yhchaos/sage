...
class Test(object):
    ...
    def setup_mm_cmd(self):
        mm_datalink_log = self.cc[1] + '_mm_datalink_run%d.log' % self.run_id
        mm_acklink_log = self.cc[1] + '_mm_acklink_run%d.log' % self.run_id
        self.mm_datalink_log = path.join(self.data_dir, mm_datalink_log)
        self.mm_acklink_log = path.join(self.data_dir, mm_acklink_log)

        if self.run_first[0] == 'receiver' or self.flows > 0:
    ...
    def prepare_tunnel_log_paths(self):
        ...
        for tun_id in range(1, self.flows + 1):
            ...
            datalink_ingress_logname = ('%s_flow%s_uid%s.log.ingress' %
                                        (self.datalink_name[tun_id-1], tun_id, uid))
            ...
            acklink_ingress_logname = ('%s_flow%s_uid%s.log.ingress' %
                                       (self.acklink_name[tun_id-1], tun_id, uid))
    ...
    def setup(self):
        ...
        self.cc_src[1] = path.join(context.src_dir, 'wrappers', self.cc[1] + '.py')
        self.cc_src[0] = path.join(context.src_dir, 'wrappers', self.cc[0] + '.py')
        self.tunnel_manager = path.join(context.src_dir, 'experiments',
                                        'tunnel_manager.py')

        # record who runs first
        if self.test_config is None:
            self.run_first[0], self.run_second[0] = utils.who_runs_first(self.cc[0])
            self.run_first[1], self.run_second[1] = utils.who_runs_first(self.cc[1])

        else:
            self.run_first[0] = None
            self.run_second[0] = None
            self.run_first[1] = None
            self.run_second[1] = None

        # setup output logs
        for ind in range(0, self.flows):
            index_cubic=0
            if ind==1:
                index_cubic=1
            self.datalink_name[ind] = self.cc[index_cubic] + '_datalink_run%d' % self.run_id
            self.acklink_name[ind] = self.cc[index_cubic] + '_acklink_run%d' % self.run_id

            self.datalink_log[ind] = path.join(
                self.data_dir, self.datalink_name[ind] + '.log')
            self.acklink_log[ind] = path.join(
                self.data_dir, self.acklink_name[ind] + '.log')
    ...
    def run_first_side(self, tun_id, send_manager, recv_manager,
        ...
        if index != 1:
            index=0
        first_src = self.cc_src[index]
        second_src = self.cc_src[index]

        if self.run_first[index] == 'receiver':
        ...
        elif self.run_first[index] == 'sender':  # self.run_first == 'sender'
    ...
    def run_second_side(self, send_manager, recv_manager, second_cmds):
        ...
        for i in range(len(second_cmds)):
            ...
            if self.run_first[index] == 'receiver':
                ...
                send_manager.stdin.flush()
            elif self.run_first[index] == 'sender':
    ...
    def download_tunnel_logs(self, tun_id):
        ...
        if self.use_geni:
#            cmd = 'scp -i %s %s:' % (path.join(path.expanduser('~'), '.ssh/id_geni_ssh_rsa'), self.r['host_addr'])
    ...
    def process_tunnel_logs(self):
        ...
        cmd = [merge_tunnel_logs, 'multiple', '-o', self.datalink_log[1]]
        if self.mode == 'local':
            cmd += ['--link-log', self.mm_datalink_log]
        cmd += datalink_tun_logs
        call(cmd)

        cmd = [merge_tunnel_logs, 'multiple', '-o', self.acklink_log[1]]
        if self.mode == 'local':
    ...
    def record_time_stats(self):
        stats_log = path.join(
            self.data_dir, '%s_stats_run%s.log' % (self.cc[1], self.run_id))
    ...
    def run(self):
        msg = 'Testing scheme %s for experiment run %d/%d...' % (
            self.cc[1], self.run_id, self.run_times)
