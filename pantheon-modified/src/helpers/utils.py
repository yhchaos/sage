...
def parse_remote_path(remote_path, cc=None, use_geni=False,key=None):
    ret = {}
    ...
    if use_geni==True:
        if key is not None:
            ret['ssh_cmd'] = ['ssh', '-i', str(key) ,ret['host_addr']]
        else:
            ret['ssh_cmd'] = ['ssh', '-i', 'id_aws_ssh_rsa', ret['host_addr']]
...
def get_git_summary(mode='local', remote_path=None, use_geni_=False,key_=None):
    git_summary_src = path.join(context.src_dir, 'experiments',
                                'git_summary.sh')
    local_git_summary = check_output(git_summary_src, cwd=context.base_dir)

    if mode == 'remote':
        r = parse_remote_path(remote_path, use_geni=use_geni_,key=key_)
...
def save_test_metadata(meta, metadata_path):
    ...
    with open(metadata_path, 'w') as metadata_fh:
            json.dump(meta, metadata_fh, sort_keys=True, indent=4, separators=(',', ': '))
