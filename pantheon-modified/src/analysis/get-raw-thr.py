...
class MakeRawData(object):
    def __init__(self, tunnel_log, out_file,ms_per_bin=500):
    ...
    def plot_delay_graph(self):
        ...
        for flow_id in self.flows:
            ...
            if flow_id in self.delays and flow_id in self.delays_t:
                ...
                ax.plot(self.delays_t[flow_id], self.delays[flow_id],
                    color=color, #marker='.',
                    label='Flow %s (90th percentile %.2f ms)'
                    % (flow_id, self.percentile_delay_90th.get(flow_id, 0)))

                #ax.scatter(self.delays_t[flow_id], self.delays[flow_id], s=1,
                ax.plot(self.delays_t[flow_id], self.delays[flow_id],
                            color=color, #marker='.',
                            label='Flow %s (Mean %.2f ms)'
                            % (flow_id, self.mean_delay.get(flow_id, 0)))
                #ax.scatter(self.delays_t[flow_id], self.delays[flow_id], s=1,
                ax.plot(self.delays_t[flow_id], self.delays[flow_id],
                            color=color, #marker='.',
                            label='Flow %s (Avg %.2f ms)'
                            % (flow_id, self.avg_delay.get(flow_id, 0)))
    ...
    def statistics_string(self):
        ...
        if self.total_avg_gput is not None:
            ret += 'Average goodput: %.2f Mbit/s\n' % self.total_avg_gput
        ...
        if self.total_90th_delay is not None:
            ret += ('90th percentile per-packet one-way delay: %.3f ms\n' %
                self.total_90th_delay)
        if self.total_jitter is not None:
            ret += ('Average per-packet one-way jitter: %.3f ms\n' %
                self.total_jitter)
        ...
        for flow_id in self.flows:
            ...
            if (flow_id in self.avg_gput and
                    self.avg_gput[flow_id] is not None):
                ret += ('Average goodput: %.2f Mbit/s\n' %
                    self.avg_gput[flow_id])
            ...
            if (flow_id in self.jitter and
                    self.jitter[flow_id] is not None):
                ret += ('Average per-packet one-way jitter: %.3f ms\n' %
                        self.jitter[flow_id])

            if (flow_id in self.percentile_delay_90th and
                    self.percentile_delay_90th[flow_id] is not None):
                ret += ('90th percentile per-packet one-way delay: %.3f ms\n' %
                        self.percentile_delay_90th[flow_id])
    ...
    def write_to_file(self):
        import csv
        for flow_id in self.flows:
            with open(str(self.out_file)+'-rr-'+str(flow_id)+'.csv', 'w') as f:
                writer = csv.writer(f, delimiter='\t')
                writer.writerows(zip(self.egress_t[flow_id],self.egress_tput[flow_id]))
            with open(str(self.out_file)+'-sr-'+str(flow_id)+'.csv', 'w') as f:
                writer = csv.writer(f, delimiter='\t')
                writer.writerows(zip(self.ingress_t[flow_id],self.ingress_tput[flow_id]))
            with open(str(self.out_file)+'-delay-'+str(flow_id)+'.csv', 'w') as f:
                writer = csv.writer(f, delimiter='\t')
                writer.writerows(zip(self.delays_t[flow_id], self.delays[flow_id]))
        with open(str(self.out_file)+'-c'+'.csv', 'w') as f:
            writer = csv.writer(f, delimiter='\t')
            writer.writerows(zip(self.link_capacity_t,self.link_capacity))
    ...
    def run(self):
        ...
        tunnel_results = {}
        tunnel_results['jitter'] = self.total_jitter
        tunnel_results['throughput'] = self.total_avg_egress
        tunnel_results['goodput'] = self.total_avg_gput
        tunnel_results['delay'] = self.total_percentile_delay
        tunnel_results['delay_90'] = self.total_90th_delay
        tunnel_results['delay_avg'] = self.total_avg_delay
        tunnel_results['delay_mean'] = self.total_mean_delay
        ...
        flow_data = {}
        flow_data['all'] = {}
        flow_data['all']['jitter'] = self.total_jitter
        flow_data['all']['tput'] = self.total_avg_egress
        flow_data['all']['gput'] = self.total_avg_gput
        flow_data['all']['delay'] = self.total_percentile_delay
        flow_data['all']['delay_90'] = self.total_90th_delay
        flow_data['all']['delay_avg'] = self.total_avg_delay
        flow_data['all']['delay_mean'] = self.total_mean_delay
...
def main():
    args = arg_parser.make_raw_data()

    raw_data = MakeRawData(
        tunnel_log=args.input_log,
        ms_per_bin=args.ms_per_bin,
        out_file=args.raw_out)
    raw_results = raw_data.run()
