...
class Plot(object):
    ...
    def eval_performance(self):
        ...
        cc_id = 0
        run_id = 1
        pool = ThreadPool(processes=multiprocessing.cpu_count()*3)
    ...
    def xaxis_linear_scale(self, ax, min_delay, max_delay):
        x_min = 0
        x_max = max_delay
    ...
    def plot_throughput_delay_linear(self, data,xlabel_,post):
        ...
        for fig_type, fig, ax in [('raw', fig_raw, ax_raw),
            ...
            self.xaxis_linear_scale(ax, min_raw_delay, max_raw_delay)
            ax.invert_xaxis()
        ...
        for graph_format in ['svg', 'pdf']:
            raw_summary = path.join(
                self.data_dir, 'pantheon_summary_linear_%s.%s' % (post,graph_format))
        ...
        for graph_format in ['svg', 'pdf']:
            mean_summary = path.join(
                self.data_dir, 'pantheon_summary_mean_linear_%s.%s' % (post, graph_format))
    ...
    def plot_throughput_delay(self, data,xlabel_,post):
        min_raw_delay = sys.maxsize
        min_mean_delay = sys.maxsize
        max_raw_delay = -sys.maxsize
        max_mean_delay = -sys.maxsize
        ...
        for graph_format in ['svg', 'pdf']:
            raw_summary = path.join(
                self.data_dir, 'pantheon_summary_%s.%s' % (post,graph_format))
            fig_raw.savefig(raw_summary, dpi=300, bbox_extra_artists=(lgd,),
                            bbox_inches='tight', pad_inches=0.2)
        ...
        for graph_format in ['svg', 'pdf']:
            mean_summary = path.join(
                self.data_dir, 'pantheon_summary_mean_%s.%s' % (post, graph_format))
            fig_mean.savefig(mean_summary, dpi=300,
                             bbox_inches='tight', pad_inches=0.2)
    ...
    def run(self):
        ...
        for cc in perf_data:
            data_for_plot[cc] = []
            data_for_plot_90[cc] = []
            data_for_plot_avg[cc] = []
            data_for_plot_mean[cc] = []
            data_for_json[cc] = {}
            ...
            for run_id in perf_data[cc]:
                ...
                gput = perf_data[cc][run_id]['goodput']
                delay = perf_data[cc][run_id]['delay']
                delay_90 = perf_data[cc][run_id]['delay_90']
                delay_avg = perf_data[cc][run_id]['delay_avg']
                delay_jitter = perf_data[cc][run_id]['jitter']
                delay_mean = perf_data[cc][run_id]['delay_mean']
                if tput is None or delay is None:
                    continue
                #data_for_plot[cc].append((gput, delay))
                #data_for_plot_90[cc].append((gput, delay_90))
                #data_for_plot_avg[cc].append((gput, delay_avg))
                #data_for_plot_mean[cc].append((gput, delay_mean))
                data_for_plot[cc].append((tput, delay))
                data_for_plot_90[cc].append((tput, delay_90))
                data_for_plot_avg[cc].append((tput, delay_avg))
                data_for_plot_mean[cc].append((tput, delay_mean))
        ...
        if not self.no_graphs:
            ...
            self.plot_throughput_delay(data_for_plot_mean,'Mean one-way delay (ms)','mean')
            self.plot_throughput_delay(data_for_plot_avg,'Avg one-way delay (ms)','avg')

            self.plot_throughput_delay_linear(data_for_plot,'95th percentile one-way delay (ms)','95')
            self.plot_throughput_delay_linear(data_for_plot_90,'90th percentile one-way delay (ms)','90')
