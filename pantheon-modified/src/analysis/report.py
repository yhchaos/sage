...
class Report(object):
    ...
    def create_table(self, data):
        align = ' c | c'
        for data_t in ['tput', 'gput', 'delay_avg','loss']:
        ...
        table = (
            ...
            '  & \\multicolumn{%(flows)d}{c|}{mean avg gput (Mbit/s)}'
#            ' & \\multicolumn{%(flows)d}{c|}{mean 95th-\\%%ile delay (ms)}'
        ...
        for cc in self.cc_schemes:
            ...
            table += (
                ...
                '%(flow_gput)s & %(flow_delays_avg)s & %(flow_losses)s \\\\\n'
            ) % {'name': data[cc]['name'],
                 'valid_runs': data[cc]['valid_runs'],
                 'flow_tputs': ' & '.join(flow_data['tput']),
                 'flow_gput': ' & '.join(flow_data['gput']),
                 'flow_delays_avg': ' & '.join(flow_data['delay_avg']),
    ...
    def summary_table(self):
        ...
        re_gput = lambda x: re.match(r'Average goodput: (.*?) Mbit/s', x)
        re_delay = lambda x: re.match(
            r'95th percentile per-packet one-way delay: (.*?) ms', x)
        re_delay_90 = lambda x: re.match(
            r'90th percentile per-packet one-way delay: (.*?) ms', x)
        re_delay_mean = lambda x: re.match(
            r'Mean per-packet one-way delay: (.*?) ms', x)
        re_delay_avg = lambda x: re.match(
            r'Average per-packet one-way delay: (.*?) ms', x)
        re_jitter = lambda x: re.match(
            r'Average per-packet one-way jitter: (.*?) ms', x)
        ...
        for cc in self.cc_schemes:
            ...
            for flow_id in range(1, self.flows + 1):
                ...
                data[cc][flow_id]['gput'] = []
                data[cc][flow_id]['delay_90'] = []
                data[cc][flow_id]['delay_avg'] = []
                data[cc][flow_id]['delay_mean'] = []
                data[cc][flow_id]['delay'] = []
                data[cc][flow_id]['jitter'] = []
            ...
            for run_id in range(1, 1 + self.run_times):
                ...
                while True:
                    ...
                    if 'Flow %d' % flow_id in line:
                        ...
                        ret = re_gput(stats_log.readline())
                        if ret:
                            ret = float(ret.group(1))
                            data[cc][flow_id]['gput'].append(ret)
                        ...
                        ret = re_jitter(stats_log.readline())
                        if ret:
                            ret = float(ret.group(1))
                            data[cc][flow_id]['jitter'].append(ret)

                        ret = re_delay_90(stats_log.readline())
                        if ret:
                            ret = float(ret.group(1))
                            data[cc][flow_id]['delay_90'].append(ret)
                        ...
                        ret = re_delay_avg(stats_log.readline())
                        if ret:
                            ret = float(ret.group(1))
                            data[cc][flow_id]['delay_avg'].append(ret)
    ...
    def include_summary(self):
        raw_summary_avg = path.join(self.data_dir, 'pantheon_summary_avg.pdf')
        mean_summary_avg = path.join(
            self.data_dir, 'pantheon_summary_mean_avg.pdf')
        raw_summary_mean = path.join(self.data_dir, 'pantheon_summary_mean.pdf')
        mean_summary_mean = path.join(
            self.data_dir, 'pantheon_summary_mean_mean.pdf')
        raw_summary_90 = path.join(self.data_dir, 'pantheon_summary_90.pdf')
        mean_summary_90 = path.join(
            self.data_dir, 'pantheon_summary_mean_90.pdf')
        raw_summary_95 = path.join(self.data_dir, 'pantheon_summary_95.pdf')
        mean_summary_95 = path.join(
            self.data_dir, 'pantheon_summary_mean_95.pdf')

        linear_raw_summary_avg = path.join(self.data_dir, 'pantheon_summary_linear_avg.pdf')
        linear_mean_summary_avg = path.join(
            self.data_dir, 'pantheon_summary_mean_linear_avg.pdf')
        linear_raw_summary_mean = path.join(self.data_dir, 'pantheon_summary_linear_mean.pdf')
        linear_mean_summary_mean = path.join(
            self.data_dir, 'pantheon_summary_mean_linear_mean.pdf')
        linear_raw_summary_90 = path.join(self.data_dir, 'pantheon_summary_linear_90.pdf')
        linear_mean_summary_90 = path.join(
            self.data_dir, 'pantheon_summary_mean_linear_90.pdf')
        linear_raw_summary_95 = path.join(self.data_dir, 'pantheon_summary_linear_95.pdf')
        linear_mean_summary_95 = path.join(
            self.data_dir, 'pantheon_summary_mean_linear_95.pdf')
        ...
        self.latex.write(
            ...
            '\\centering\n'
            '\\includegraphics[width=0.95\\linewidth,height=2.2in]{#1}\n'
            '\\end{minipage}\n'
            '\\hfill}\n\n'
            '\\newcommand{\PantheonFig}[1]{%%\n'
            '\\begin{figure}[H]\n'
            '\\centering\n'
            '\\IfFileExists{#1}{\\includegraphics[width=0.5\\textwidth,height=2.5in]{#1}}'
            '{Figure is missing}\n'
            '\\end{figure}}\n\n'
            '\\begin{document}\n'
            '%s'
...
#            '\\PantheonFig{%s}\n\n'
            ...
            % (metadata_desc,  mean_summary_avg, raw_summary_avg, mean_summary_mean, raw_summary_mean, mean_summary_95, raw_summary_95, mean_summary_90, raw_summary_90
               ,linear_mean_summary_avg, linear_raw_summary_avg, linear_mean_summary_mean, linear_raw_summary_mean,
               linear_mean_summary_95, linear_raw_summary_95, linear_mean_summary_90, linear_raw_summary_90))
